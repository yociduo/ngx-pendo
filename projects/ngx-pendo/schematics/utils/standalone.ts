...
export function importsProvidersFrom(tree: Tree, filePath: string, className: string): boolean {
  ...
  return !!importProvidersFromCall?.arguments.some(
    (arg: ts.Expression) => ts.isIdentifier(arg) && arg.text === className
  );
}
 ...
 */
export function callsProvidersFunction(tree: Tree, filePath: string, functionName: string): boolean {
  const sourceFile = createSourceFile(tree, filePath);
  const bootstrapCall = findBootstrapApplicationCall(sourceFile);
  const appConfig = bootstrapCall ? findAppConfig(bootstrapCall, tree, filePath) : null;
  ...
  return !!providersLiteral?.elements.some(
    (el: ts.Expression) =>
      ts.isCallExpression(el) && ts.isIdentifier(el.expression) && el.expression.text === functionName
  );
}
...
export function addModuleImportToStandaloneBootstrap(
  ...
  if (!appConfig) {
    throw new SchematicsException(`Could not statically analyze config in bootstrapApplication call in ${filePath}`);
  }
...
function findProvidersLiteral(config: ts.ObjectLiteralExpression): ts.ArrayLiteralExpression | null {
 ...
 */
function findAppConfig(bootstrapCall: ts.CallExpression, tree: Tree, filePath: string): ResolvedAppConfig | null {
  if (bootstrapCall.arguments.length > 1) {
 ...
 */
function findImportLocalName(sourceFile: ts.SourceFile, name: string, moduleName: string): string | null {
  for (const node of sourceFile.statements) {
 ...
 */
function addNewAppConfigToCall(call: ts.CallExpression, expression: ts.Expression, recorder: UpdateRecorder): void {
 ...
 * @param recorder Recorder to which to log the change.
 */
function addElementToArray(node: ts.ArrayLiteralExpression, element: ts.Expression, recorder: UpdateRecorder): void {
